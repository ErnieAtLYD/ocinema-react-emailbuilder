describe("Basic row stuff", function() {
  beforeEach(function() {
    cy.visit("http://localhost:3000/");
    cy.get("[data-cy=add-1-col]").click();
  });
...
describe("Drag and drop element to 1 column", function() {
  beforeEach(function() {
    cy.visit("http://localhost:3000/");
    cy.get("[data-cy=add-1-col]").click();
    cy.get("[data-cy=dragmehandle]").dragTo(".column");
    cy.get(".column")
      .eq(0)
      .as("firstColumn");
  });
...
describe("Drag and drop element to 2 columns", function() {
  beforeEach(function() {
    cy.visit("http://localhost:3000/");
    cy.get("[data-cy=add-1-col]").click();
    cy.get("[data-cy=add-1-col]").click();
    ...
    cy.get(".column")
      .eq(1)
      .as("secondColumn");
    cy.get("[data-cy=dragmehandle]").dragTo("@firstColumn");
  });
  ...
  describe("elements on elements", function() {
    beforeEach(function() {
      cy.get(".element").dragTo("@secondColumn");
      cy.get("[data-cy=dragmehandle]").dragTo("@secondColumn");
      cy.get("[data-cy=dragmehandle]").dragTo("@secondColumn");
      cy.get("[data-cy=dragmehandle]").dragTo("@secondColumn");
      cy.get("@secondColumn")
        .find(".element")
    ...
    it("drag and drops an element on an element in a different column", function() {
      ...
      cy.get("@secondElement")
        .invoke("text")
        .then(text1 => {
          cy.get("@firstColFirstElement").dragTo("@secondElement");
